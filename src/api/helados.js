...
// Obtener todos los helados (versión corregida)
// Esta función devuelve un objeto Response, no JSON
...
// Esta función devuelve un objeto JSON directamente
export const getHeladoById = async (id) => {
  try {
    ...
    // Asegúrate que la respuesta tenga esta estructura
    return {
      id: data.id,
      nombreHelado: data.nombreHelado || data.nombre || "",
      descripcion: data.descripcion || "",
      precio: data.precio || 0,
      itsArtesanal: data.itsArtesanal || false,
      categoriaId: data.categoriaId || data.categoria?.id || "",
      nombreCategoria: data.nombreCategoria || data.categoria?.nombre || "",
      estadoId: data.estadoId || data.estado?.id || "",
      nombreEstado: data.nombreEstado || data.estado?.nombre || "",
      ingredients: data.ingredients || data.ingredientes || [], // Asegura array
    };
  } catch (error) {
...
export const addIngredienteToHelado = async (heladoId, ingredienteId) => {
  try {
    ...
    if (isNaN(id)) throw new Error(`ID de ingrediente inválido: ${ingredienteId}`);
...
export const updateHeladoIngredients = async (heladoId, selectedIngredientIds) => {
  try {
